...
const Dashboard: React.FC = () => {
    ...
    const fetchDashboard = async () => {
      try {
        ...
        const reqRes = await fetch(
          import.meta.env.VITE_API_URL + "/api/therapy-requests/my",
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
  ...
  if (!dashboardData) {
    return (
      <div className="min-h-[100dvh] flex items-center justify-center bg-warm-50 text-sage-500 font-medium">
        Loading dashboard...
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8 flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          <h1 className="font-serif text-3xl font-bold text-sage-800 mb-2">
            Welcome back, {user?.name.split(" ")[0]}
          </h1>
          <p className="text-sage-400">
            Here's an overview of your wellness journey this week.
          </p>
        </div>
      </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-3xl p-6 mb-8 flex flex-col md:flex-row items-center justify-between shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-start gap-5 relative z-10 mb-6 md:mb-0">
          ...
          <div>
            ...
            <p className="text-red-600/80 text-sm font-medium mb-3">
              If you are in immediate danger or experiencing a crisis, support
              is available 24/7.
            </p>
            ...
            <div className="flex flex-wrap gap-3">
              <a
                href="tel:988"
                className="inline-flex items-center gap-2 bg-white text-red-600 px-4 py-2 rounded-xl text-sm font-bold shadow-sm border border-red-100 hover:bg-red-50 transition-colors"
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          ...
          value={`${stats.mindfulnessMins} min`}
          color="text-sage-500"
          bgColor="bg-sage-50"
        />
        <StatCard
          ...
          label="Sessions"
          value={`${stats.sessionsCompleted} Completed`}
          color="text-warm-500"
          bgColor="bg-warm-50"
        />
        <StatCard
          ...
          value={`${stats.journalEntries} this week`}
          color="text-clay-500"
          bgColor="bg-clay-50"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-white border border-sage-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-sage-800 mb-6">
              Mood History
            </h3>
          ...
          <div className="bg-white border border-sage-100 rounded-2xl p-6 shadow-sm">
            ...
            {upcomingSession ? (
                ...
                <div className="bg-sage-50 rounded-xl p-4 flex items-center gap-4 border border-sage-100 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-sage-500 font-bold">
                      {new Date(upcomingSession.date).toLocaleDateString()} at{" "}
              ...
              <div className="text-center py-6 px-4 bg-sage-50/50 rounded-2xl border border-sage-200 border-dashed">
                ...
                <h4 className="font-bold text-sage-600 text-sm mb-1">
                  No sessions booked
                </h4>
                <p className="text-xs text-sage-400 mb-4">
                  Schedule a time to speak with a professional.
                </p>
                <a
                  href="/therapists"
                  className="px-4 py-2 bg-white border border-sage-200 text-sage-600 rounded-lg text-xs font-bold hover:bg-sage-50 shadow-sm block"
                >
                  Find a Therapist
                </a>
              </div>
          ...
          <div className="lg:col-span-3 mt-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {therapyRequests.map((req) => (
                <div
                  key={req.id}
                  className="bg-white border border-sage-100 rounded-2xl p-5 shadow-sm"
                >
                  <div className="flex items-center justify-between mb-3">
                    <span
                      className={`px-2.5 py-1 text-[10px] font-bold rounded-lg border uppercase tracking-wider ${
                        req.status === "pending"
                          ? "bg-warm-50 text-warm-600 border-warm-100"
                          : req.status === "approved"
                    ...
                    <span className="px-2 py-0.5 bg-ocean-50 text-ocean-600 text-[10px] font-bold rounded-md border border-ocean-100 uppercase">
                      {req.domain}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1 mb-4">
                    <div
                      className={`h-1.5 flex-1 rounded-full ${req.status !== "rejected" ? "bg-sage-500" : "bg-clay-300"}`}
                    ></div>
                    <div
                      className={`h-1.5 flex-1 rounded-full ${["approved", "scheduled", "completed"].includes(req.status) ? "bg-sage-500" : "bg-sage-200"}`}
                    ></div>
                    <div
                      className={`h-1.5 flex-1 rounded-full ${["scheduled", "completed"].includes(req.status) ? "bg-sage-500" : "bg-sage-200"}`}
                    ></div>
                  </div>

                  {req.therapist_name && (
                    <p className="text-sm text-sage-700 font-semibold mb-1">
                  ...
                  {req.scheduled_date && (
                    <div className="flex items-center gap-2 text-sm text-ocean-600 font-medium bg-ocean-50/50 px-3 py-2 rounded-lg border border-ocean-100">
                      ...
                      {new Date(
                        req.scheduled_date,
                      ).toLocaleDateString()} at {req.scheduled_time}
                    </div>
                  )}
                  {req.status === "pending" && (
                    <p className="text-xs text-sage-400 mt-2">
                      Waiting for a therapist to review...
                    </p>
                  )}
                  {req.status === "scheduled" && (
                    <Link
  ...
  return (
    <div className="bg-white border border-sage-100 rounded-2xl p-6 flex flex-col items-center justify-center min-h-[250px] relative overflow-hidden shadow-sm">
      ...
      >
        ...
        <span className="text-ocean-700 font-bold text-sm relative z-20">
          {phase}
        </span>
      </div>
      <p className="text-xs text-sage-400 mt-8 font-medium">
  ...
  return (
    <div className="bg-white border border-sage-100 rounded-2xl p-6 flex flex-col items-center min-h-[250px] shadow-sm">
      <div className="w-full flex justify-between items-center mb-4">
        ...
        <button
          onClick={reset}
          className="text-sage-300 hover:text-sage-500 transition-colors bg-sage-50 p-1.5 rounded-lg"
          title="Reset"
        >
          <RotateCcw size={14} />
        </button>
      ...
      <div
        className={`mt-4 text-xs font-bold text-clay-500 transition-opacity duration-500 ${allPopped ? "opacity-100" : "opacity-0"}`}
      >
...
const StatCard: React.FC<{
  ...
  <div className="bg-white border border-sage-100 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
    <div className={`flex items-center gap-3 mb-3`}>
      <div
        className={`w-10 h-10 rounded-xl ${bgColor} ${color} flex items-center justify-center`}
      >
        {icon}
      </div>
